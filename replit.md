...
| **Sensor de injeção rebobinado: o cabo do computador de bordo agora chega numa centralinha auxiliar plugada no encaixe vazio (mesma "marca" da bomba aftermarket da PARTE 7). A cada ciclo do motor, ela toggla a flag de "frame pronto", incrementa o tacômetro principal e o tacômetro auxiliar — exatamente como o sensor original faria** | **PARTE 8 PLANO A: stub C++ `gow_intc_handler_0x182f28` em `game_overrides.cpp` registrado como handler INTC VBlank em `0x00182F28`. Replica fiel das 8 instruções MIPS perdidas pelo PS2Recomp: toggle `[0x29C7D8]^=1`, `[0x29C7D4]+=1`, `[0x334F58]+=1`. Build incremental ~30s** | ✅ **CONFIRMADO 2026-04-27 PARTE 8 — Cenário A (Vitória total): stub registrou, VBlank tick #1..#300 (5s @ 60Hz), `[INTC:skip]` sumiu, `[vif1:cmd]` apareceu com IRQ no idx=20 (VIF1 começou). Bug G DEFINITIVAMENTE RESOLVIDO** |
| **Quatro botões pneumáticos no painel da bancada do mecânico ficaram apertando sozinhos esperando luz verde — fonte de alimentação dos botões nem foi instalada na fábrica (4 buracos no switchboard de syscalls 0x79/0x7A/0x7B/0x7D)** | **Bug H: caller `entry_296518:0x296710` chama `func_294030` (= syscall 0x7A) num poll loop `and $v0,$v0,$s0; beqz $v0,-0x4` esperando bit 17 do retorno. Switch `dispatchNumericSyscall` em `ps2_syscalls.cpp` não tinha cases pra 0x79-0x7D, handler default `TODO` retornava 0 → 130k hits/12s no baseline log_part9** | 🟡 **identificado e atacado PARTE 9** |
| **Mecânico ligou os 4 botões à fonte central e configurou todos pra responderem "TODOS OS BITS OK" (qualquer pergunta de máscara bit-a-bit do retorno produz != 0). 1ª chamada de cada botão é registrada no manual de bordo pra auditoria futura** | **PARTE 9 PLANO A: 4 cases novos (0x79/0x7A/0x7B/0x7D) no switch de `ps2_syscalls.cpp:321-363`, retornando `-1` (= `0xFFFFFFFF`). Log da 1ª chamada de cada syscall com mutex + `unordered_set` (padrão `s_unknownCounts` da TODO). Build incremental `rebuild_runtime.sh` ~30s** | ✅ **CONFIRMADO 2026-04-27 PARTE 9 — Vitória total: stub disparou 1x (linha 393 do log_part9b), `Unknown syscallId=0x7a` SUMIU (de 40k+ hits/12s pra 0), VBlank avançou pra #960 (16s @ 60Hz, +220% vs PARTE 8), 7 novos `CreateSema id=4..10` criados, jogo entrou em território novo. Bug H DEFINITIVAMENTE RESOLVIDO** |
| **Motor agora gira 16s liso, mas o caminhão de carga entre o cofre principal e o cofre auxiliar (SIF DMA EE↔IOP) tenta sair 8 vezes e o motor da embreagem não pega — log diz só "failed" sem dizer POR QUÊ** | **Bug I (suspeito): `sceSifSetDma` em `ps2_stubs_misc.inl:3261` está implementado mas rejeita silenciosamente. Log atual `sceSifSetDma failed dmat=0x1fffed0 count=0x1` (8x) não revela qual validação falha (`!entry`, `size>RAM`, `canCopyGuestByteRange`, `copyGuestByteRange`) nem os endereços `xfer.src`/`xfer.dest`/`xfer.size`/`xfer.attr`. Sem isso, o fix definitivo seria chute** | 🟡 **identificado e instrumentado PARTE 10 PLANO A** |
| **Mecânico instalou um sensor diagnóstico no acoplamento do caminhão: a próxima vez que tentar sair, vai gravar EXATAMENTE qual válvula travou + endereço de origem, destino, tamanho da carga e tipo de transferência. Sem mudar comportamento — só ouvinte. Comportamento atual mantido pra próxima rodada de teste revelar a causa raiz** | **PARTE 10 PLANO A: instrumentação cirúrgica em `sceSifSetDma` — variáveis `failIndex`, `failReason` (string identificando qual ifguard rejeitou), `failXfer` (struct completa). Logado com mutex + `unordered_set` nas primeiras 8 combinações únicas `(dest,src)` pra evitar inundação. Log antigo "sceSifSetDma failed" preservado. Build incremental ~30s** | ✅ **CONFIRMADO 2026-04-27 PARTE 10 (log_part10.txt do Agente Cris, 437 linhas, cancelado em VBlank #1680/28s).** Câmera capturou: **`xfer.dest = 0xffffffff` em todas as 8 falhas**, `motivo='canCopyGuestByteRange_rejeitou_dest_ou_src'`. `xfer.src` válido (`0x327880` 1ª vez, depois `0x20327ac0` = mirror KSEG1). Hipótese: `dest=0xffffffff` é sentinela "stage transfer" do SIF RPC — EE não conhece destino, IOP decide pelo header do pacote. Runtime atual rejeita por validar endereço cru. Próxima ação: PARTE 10 PLANO B (B3=dump conteúdo do pacote em `xfer.src`, B1=stub permissivo, B2=implementar SIF RPC stage). |
...
| **Recrutamos um agente de tráfego pro cruzamento — ele tem o mesmo crachá que o sinaleiro original mas é nosso. Investigação revelou que o "sinal nunca chega" foi falha do PS2Recomp: as 8 instruções do sinaleiro original ESTÃO no código transcrito (`sub_00182EE8`, linhas 134-196), mas vêm depois de um `jr $ra` em `0x182f20` — o detector de funções parou ali e nunca registrou `0x182f28` como entry. Nosso agente substituto replica fielmente as 3 escritas que o sinaleiro fazia: toggle da flag de "pista limpa" (`[0x29C7D8]`), incrementa o contador principal de carros (`[0x29C7D4]`) e o contador auxiliar (`[0x334F58]`). Cada batida do worker thread de VBlank do runtime (60 Hz) agora dispara o agente, que pisca o sinal e libera o `sub_0021ff60` pra prosseguir** | **PARTE 8 PLANO A: stub C++ `gow_intc_handler_0x182f28` em `game_overrides.cpp` registrado via `runtime.registerFunction(0x00182F28, ...)`. Registro em paralelo ao stub PARTE 7 dentro de `apply_god_of_war_overrides`. Build incremental ~30s** | ✅ **CONFIRMADO 2026-04-27 PARTE 8 — Vitória total: 720 piscadas (12s @ 60Hz) no dossiê do Agente Cris, [INTC:skip] sumiu, agente avançou ATÉ a próxima sala** |
| **Próxima sala: 4 portas com leitor biométrico (syscalls SIF poll 0x79/0x7A/0x7B/0x7D), todos OFFLINE — switchboard central não tinha entrada pra eles, handler default respondia "ACESSO NEGADO". Agente principal martelou o leitor 0x7A 130.000 vezes em 12 segundos esperando luz verde no bit 17 (`and $v0,$v0,0x20000; beqz $v0,-0x4`)** | **Bug H: 3 callers distintos (RA=0x2966c4 com `$a0=0x80000000`, RA=0x296718 com `$a0=0x4` LOOP, RA=0x2703412 com `$a0=0x4 $a3=0x32`) — todos travados no mesmo poll. Wrappers MIPS triviais `addiu $v1,N; syscall; jr $ra` em `entry_294020`/`entry_294030`/`entry_294040` confirmados** | 🟡 **identificado e atacado PARTE 9** |
| **Plantamos um operador no switchboard que reconhece os 4 leitores e responde "TODOS OS BITS OK" pra qualquer pergunta. Loga só a 1ª chamada de cada um pra rastreabilidade. Os 3 callers da 0x7A são cobertos pelo mesmo operador num único movimento** | **PARTE 9 PLANO A: 4 cases novos no switch de `dispatchNumericSyscall` em `ps2_syscalls.cpp:321-363`, retornando `-1` (= `0xFFFFFFFF`); log 1ª chamada com mutex + `unordered_set`. Cobre os 3 callers de uma vez. Custo: `bash rebuild_runtime.sh` ~30s** | ✅ **CONFIRMADO 2026-04-27 PARTE 9 — Cenário A (vitória total): operador respondeu na 1ª biometria, agente atravessou a sala, criou 7 crachás auxiliares (`CreateSema id=4..10`) e chegou no portão de carga do SIF a 16 segundos de execução. Bug H ENCERRADO** |
| **Próxima sala: portão de carga do SIF (Sub-system Interface entre o cofre EE e o cofre IOP). Agente tentou despachar 8 caminhões de carga e a embreagem não pegou — `sceSifSetDma failed` repetindo. Mas a câmera do portão só registra "tentativa falhou", não diz QUAL válvula travou nem PRA ONDE o caminhão queria ir** | **Bug I (suspeito): stub `sceSifSetDma` em `ps2_stubs_misc.inl:3261` valida descritor (16B: src/dest/size/attr) e rejeita silenciosamente quando `getConstMemPtr`/`canCopyGuestByteRange`/`copyGuestByteRange` retornam falha. Provável causa: `xfer.src` ou `xfer.dest` aponta pra memória IOP/scratchpad/kernel (fora dos 32 MB de RAM principal). Sem dado, é chute** | 🟡 **identificado e instrumentado PARTE 10 PLANO A** |
| **Mecânico do portão instalou um sensor diagnóstico passivo: na próxima tentativa, vai gravar EXATAMENTE qual válvula falhou (`entryAddr_fora_da_RAM` / `size_maior_que_RAM` / `canCopyGuestByteRange_rejeitou_dest_ou_src` / `copyGuestByteRange_falhou_durante_copia`) + endereço src/dest/size/attr completos + RA/PC do caller. Limita a 8 combinações únicas (dest,src) pra não inundar. Comportamento mantido: caminhão continua não saindo, mas agora a câmera mostra TUDO** | **PARTE 10 PLANO A: instrumentação cirúrgica em `sceSifSetDma`. Variáveis `failIndex`/`failReason`/`failXfer` preenchidas no ponto da falha; log detalhado via `std::mutex` + `std::unordered_set<uint64_t>` chaveado em `(dest<<32)^src`. Log antigo "failed dmat= count=" preservado pra contagem. Build `rebuild_runtime.sh` ~30s** | ✅ **CONFIRMADO 2026-04-27 PARTE 10 (log_part10.txt, 437 linhas, cancelado em VBlank #1680/28s).** Causa raiz revelada: `xfer.dest = 0xffffffff` (sentinela U32 "-1") em **todas** as 8 falhas. `xfer.src` válido (`0x327880` na 1ª chamada via `dmat=0x1fffed0 size=0x14 attr=0x44`, depois `0x20327ac0` = mirror KSEG1 via `dmat=0x1fffdf0 size=0x40 attr=0x44`). RA constante `0x2969ac`, PC `0x293fe4`. Programa entrou em loop estável (CreateSema id=4..17, VBlank avançando), não trava o jogo mas trava o subsistema SIF. Hipótese: `dest=0xffffffff` é sentinela de "stage transfer" do SIF RPC (EE manda pacote sem destino fixo, IOP decide pelo header). Próxima ação: PARTE 10 PLANO B — escolher entre **B1** (stub permissivo: aceitar `dest==0xffffffff`, retornar sucesso fake), **B2** (implementar SIF RPC stage de verdade), **B3** (instrumentação extra: dump em hex dos primeiros bytes em `xfer.src` antes do log de falha — confirma se header é `RPC_BIND` da Sony). Recomendação: B3 primeiro (zero risco, ~10 linhas, decide entre B1 e B2). |
| Próximos guardas internos previstos | VIF1/DMA com payloads válidos, `SetupThread`, restantes do init chain, INTC handlers, GS, áudio, gamepad | 🔜 ato 3 |
...
#### 🆕 SESSÃO 2026-04-27 PARTE 7 — VITÓRIA TOTAL Cenário A (Bug F DEFINITIVAMENTE RESOLVIDO)

**Status:** 🏆 **VITÓRIA TÁTICA DEFINITIVA**. Stub C++ do alocador-fantasma `sub_0013DA10` aplicado em `game_overrides.cpp` e CONFIRMADO funcionando pelo Agente Cris (log_part7.txt, 382 linhas, 2026-04-27). O stub virou o agente duplo no balcão do almoxarifado: entrega fichas reais vindas de um cofre paralelo (1 MB reservado em `[0x01000000..0x01100000]` da RAM PS2 emulada), os 4 agentes de campo arquivaram corretamente no fichário circular, supervisor `13FAB8` percorreu a fila inteira em 3 iterações limpas. **A blindagem da PARTE 6 nem precisou disparar — virou rede de segurança em estado dormente.**

**Resultados do log_part7.txt:**

| Sinal | Status | Detalhe |
|---|---|---|
| `[game_overrides] applying 'GodOfWarPCPort:sub_0013DA10'` | ✅ Disparou | Match do ELF SCUS_973.99 OK |
| `[game_overrides] God of War: stub PARTE 7 PLANO A registrado em 0x0013DA10 — bump allocator de 16384 nos em [0x1000000..0x1100000]` | ✅ | Override registrado antes de `Iniciando execução...` |
| `[stub:sub_0013DA10] PARTE 7 PLANO A: alloc #N` | ✅ **4 disparos** | guestPtr=0x1000000, 0x1000040, 0x1000080, 0x10000c0 (todos com `a0=0x0 ra=0x13fb14`) |
| `[BLINDAGEM PARTE 6]` | ✅ **ZERO disparos** | Stub fornece $v0 válido — blindagem nunca acionada |
| `[watch:SENTINEL_0x2cbbb0]` | ✅ **5 escritas com VALORES VÁLIDOS** | prev=0x1000000 (PC 0x13fb24), next=0x1000000 (0x13fb3c), next=0x1000040 (0x13fb3c), next=0x1000080 (0x13fb3c), prev=0x10000c0 (0x13fb24) — lista circular real |
| `[13FAB8] loop iter=N` | ✅ **3 iterações** | s0=0x1000040 → 0x1000000 → 0x2cbbb0 (volta na sentinela). Primeira vez que esse loop completa NORMALMENTE percorrendo lista real |
| `entry_182ff0 COMPLETA` | ✅ | Mantido da PARTE 6, agora com estruturas internas reais |
| Tempo de build | ~30s | `bash rebuild_runtime.sh` apenas relinka ps2runtime + GodOfWarPCPort, NÃO recompila os 5.625 .cpp |
| Programa rodou | 2× consecutivas | (Agente Cris executou duas vezes — comportamento idêntico em ambas) |

**Mudanças aplicadas nesta sessão (PARTE 7 PLANO A):**

`PS2Recomp/ps2xRuntime/src/lib/game_overrides.cpp` (linhas 253-340):
- Includes adicionados: `ps2_runtime_macros.h` (pra `WRITE32`/`SET_GPR_U32`/`GPR_U32`) e `<atomic>`
- Constantes: `kGowStubHeapBase = 0x01000000u`, `kGowStubHeapSize = 0x00100000u` (1 MB), `kGowStubNodeBytes = 64u`
- Variáveis estáticas atômicas: `g_gowStubHeapOffset`, `g_gowStubAllocCount`
- Função `gow_stub_sub_0013DA10(rdram, ctx, runtime)`:
  - `fetch_add(64)` atômico no offset
  - Se ultrapassar 1 MB: loga warning, retorna `$v0=0` (blindagem PARTE 6 cuida — defesa em profundidade)
  - Senão: zera o nó com `WRITE32` (16 escritas de 4 bytes), seta `$v0=guestPtr`, `pc=$ra`
  - Loga primeiros 16 disparos com `a0` e `ra` do chamador
- Função `apply_god_of_war_overrides(runtime)`: chama `runtime.registerFunction(0x0013DA10u, gow_stub_sub_0013DA10)`
- `PS2_REGISTER_GAME_OVERRIDE("GodOfWarPCPort:sub_0013DA10", "SCUS_973.99", 0u, 0u, apply_god_of_war_overrides)`

**Mapa de RAM PS2 documentado (importante pra próximas sessões):**
```
[0x00000000 .. 0x00100000)  reservado/syscalls
[0x00100000 .. 0x0035d080)  ELF carregado (God of War)
[0x0035d080 .. 0x01000000)  heap nativa do jogo
[0x01000000 .. 0x01100000)  <-- RESERVADO PARA O STUB PARTE 7 (1 MB)
[0x01100000 .. 0x01F00000)  livre (espaço pra futuros stubs)
[0x01F00000 .. 0x02000000)  stack típica
```

**Diagnóstico Bug G refinado pela PARTE 7:**
- `ls GOD_PC_PORT_FINAL/src/recompiled/sub_*0x182f28*.cpp` → **NÃO EXISTE**
- `cause=2` no INTC do PS2 = **VBlank Start** (sincronização de quadro vertical)
- Handler é alcançado via tabela de handlers de interrupção (não via JAL direto), então o PS2Recomp não conseguiu identificá-lo como função → ficou de fora dos 5.625 arquivos gerados
- `sub_0021ff60.cpp` (a função do JAL [7/11]) **EXISTE** — está aguardando o contador de VBlank avançar pra prosseguir; como ninguém atende a interrupção, o contador nunca muda → loop infinito
- As 4 funções "NÃO REGISTRADA" do init pre-main (`0x283770`, `0x17acb8`, `0x138b10`, `0x1838d0`) também NÃO existem nos 5.625 .cpp — confirmando que é uma falha sistemática de detecção do PS2Recomp pra funções alcançadas indiretamente

#### 🎯 PLANO DA PARTE 8 (próxima sessão) — atacar Bug G

**Opção A8 — Stub C++ pro `sub_00182f28` em `game_overrides.cpp` (recomendada primeiro):**
- Mesmo padrão da PARTE 7 que acabou de funcionar perfeitamente
- Tempo de build: **~30s** com `rebuild_runtime.sh`
- Conteúdo do stub: minimalista — apenas marcar IRQ como "atendido" e retornar (talvez bumpando algum contador de VBlank se descobrirmos onde fica)
- Risco: se o handler real fizer cleanup específico de DMA/GS, stub minimalista pode quebrar coisas
- Pré-requisito: investigar antes onde no runtime está o ponto que loga `[INTC:skip]` (provavelmente `ps2_runtime.cpp` ou `ps2_intc.cpp`) pra entender o protocolo de "atender IRQ"

**Opção B8 — Disassembly + reescrita manual do `0x182f28` em C++:**
- Ler bytes do ELF em `[0x182f28..0x182fXX]`, desassemblar, transcrever em C++
- Tempo: maior esforço, mas resolve causa raiz E provavelmente desbloqueia outras funções "fantasma" do mesmo padrão (as 4 NÃO REGISTRADAS do init pre-main)
- Tempo de build: ~30s (só edita game_overrides.cpp)

**Opção C8 — "Fake-atender" IRQs no `[INTC:skip]` mechanism (radical):**
- Modificar o ponto do runtime que loga `[INTC:skip]` pra incrementar contadores típicos de VBlank/HBlank em vez de só pular silenciosamente
- Resolve apenas o sintoma, mas pode destravar muitos jogos com a mesma classe de problema (potencial fix universal)
- Risco: pode mascarar outros bugs

**Recomendação do analista:** começar por **Opção A8** (mesmo padrão da PARTE 7 acabou de funcionar). Antes de aplicar, investigar 2 pontos no runtime:
1. Onde está o código que emite `[INTC:skip]` (pra entender o protocolo de retorno de IRQ)
2. Se já existe um contador de VBlank exposto que o stub poderia bumpar
...
#### 🎯 PLANO DA PARTE 7 — ✅ EXECUTADO (resultado documentado na seção PARTE 7 acima)

**Decisão tomada pelo Agente Cris:** Plano A (Bug F primeiro). Aplicado e CONFIRMADO funcionando — ver seção `🆕 SESSÃO 2026-04-27 PARTE 7` no topo desta cadeia.

**Resumo do que foi feito:** stub C++ `gow_stub_sub_0013DA10` em `PS2Recomp/ps2xRuntime/src/lib/game_overrides.cpp` — bump allocator atômico em `[0x01000000..0x01100000]` (1 MB, 16.384 nós), registrado via `PS2_REGISTER_GAME_OVERRIDE`. Build incremental ~30s.

**Resultado:** 4 allocs reais, lista circular montada, blindagem PARTE 6 nunca disparou. Bug F DEFINITIVAMENTE RESOLVIDO. Próximo alvo: Bug G (PARTE 8).
...
| **H** | **Família de syscalls SIF poll faltando no switch → spinloop bit-mask** | Loop infinito sem crash, log explode com `Unknown syscallId=0xN hits=...` (10k-130k em segundos) | Switch `dispatchNumericSyscall` em `ps2_syscalls.cpp` tinha buracos em 0x79/0x7A/0x7B/0x7D entre `sceSifSetDChain` (0x78) e `GetMemorySize` (0x7F). Caller faz `jal wrapper; and $v0,$v0,$sN; beqz $v0,-0x4` → poll de bit que nunca acende com retorno default 0 | ✅ **PARTE 9 RESOLVIDO** (confirmado log_part9b 2026-04-27): 4 cases novos no switch retornando `-1` (= `0xFFFFFFFF`, destrava qualquer máscara bit) + log 1ª chamada com mutex + `unordered_set`. Resultado: stub disparou 1x, `Unknown syscallId=0x7a` SUMIU (40k+ hits/12s → 0), VBlank avançou pra #960 (16s @ 60Hz, +220% vs PARTE 8) | Log mostra `Unknown syscallId=0xN hits=10000+` crescendo rápido + caller MIPS típico `jal wrapper; and $v0,$v0,$sN; beqz $v0,-0x4` |
| **I** | **Stub SIF DMA EE↔IOP rejeita silenciosamente sem dizer qual validação falhou** | `sceSifSetDma failed dmat=0xN count=0xM` repete 8+ vezes em segundos (não trava o jogo, mas impede transferência EE↔IOP de continuar) | Stub `sceSifSetDma` em `ps2_stubs_misc.inl:3261` valida descritor 16B (src/dest/size/attr) via 4 ifs (`!entry`, `size>RAM`, `canCopyGuestByteRange`, `copyGuestByteRange`). Em falha, retorna 0 sem dizer QUAL if rejeitou nem QUAIS endereços o jogo mandou. **Causa raiz revelada PARTE 10 PLANO A:** `xfer.dest = 0xffffffff` (sentinela U32 "-1") em todas as 8 falhas; `xfer.src` válido (`0x327880` na 1ª chamada, depois `0x20327ac0` = mirror KSEG1). `motivo='canCopyGuestByteRange_rejeitou_dest_ou_src'` (if 3). Hipótese forte: `dest=0xffffffff` é sentinela "stage transfer" do SIF RPC — EE manda pacote sem destino fixo, IOP decide pelo header (`RPC_BIND`/`RPC_CALL`). Runtime atual rejeita por validar endereço cru. RA constante `0x2969ac`, PC `0x293fe4` (1 único call site no ELF). | ✅ **PARTE 10 PLANO B1 CONFIRMADO 2026-04-27 (log_part10d.txt 491 linhas).** B1 disparou 4x reais (linhas 402, 414, 534, 537), as 8 mensagens `motivo='canCopyGuestByteRange...'` SUMIRAM totalmente, jogo estável em VBlank #2460 (~41s no 2º run), zero crashes/syscalls faltando/SIGSEGV. **🆕 BUG J DESCOBERTO** (linhas 406, 535 do log_part10d): chamada SIF "marciana" `sceSifSetDma falhou — dmat=0x5 count=0x20 idxFail=1 motivo='size_maior_que_RAM' xfer.src=0x0 xfer.dest=0x0 xfer.size=0x20000000 xfer.attr=0x0 ra=0x0 pc=0x293fe4`. **`ra=0x0` é smoking gun** (impossível em MIPS legítimo — todo JAL seta $ra). Hipótese: stub não-recompilado (`0x283770`, `0x17acb8`, `0x138b10`, `0x1838d0` — vimos como "NAO REGISTRADA - skip" no log) deixa registers em estado lixo, controle volta pra código recompilado e cai num JAL com argumentos zerados. 🟡 **PARTE 10 PLANO C APLICADO 2026-04-27** (`ps2_stubs_misc.inl:3287-3313`): blindagem `if (dmatAddr < 0x100000) return 0` (mantém comportamento idêntico ao caso `dmatAddr==0`, só silencia 4 warnings de ruído por run) + log UMA VEZ dos registers completos (`a0`, `a1`, `ra`, `sp`, `gp`, `pc`) pra rastrear origem do Bug J. Threshold 0x100000 seguro: chamadas legítimas usam `dmat=0x1fffed0/0x1fffdf0` (>1MB). | ~~PARTE 10 PLANO A CONFIRMADO 2026-04-27~~ ~~PARTE 10 PLANO B3 CONFIRMADO 2026-04-27~~ Câmera B3 capturou os 2 padrões de pacote em hex: **1º pacote (20 bytes em 0x327880) = `[size=0x14, 0, 0x80000000, 0, 0x003277c0]`** (header SIF Sony opcode=0, provável SIF_CMD_INIT/handshake). **2º pacote (64 bytes em 0x20327ac0) = `[size=0x40, 0, 0x80000009, 0, rpc_id=5, self-ptr, payload_words=2, response_buf=0x0030aaa8, sub-header=0x80000592, ...]`** (header SIF Sony opcode=9, provável RPC_BIND ou RPC_CALL). **Confirmado: protocolo SIF RPC genuíno da Sony, não estrutura caseira do GoW.** 🟡 **PARTE 10 PLANO B1 APLICADO 2026-04-27** (escolhido pela central após B3 confirmar SIF RPC: B1 atalho ~25 linhas é mais viável que B2 fix completo ~200-400 linhas porque revela próximo travamento em ~30s vs decifrar protocolo Sony inteiro): adicionado early-return em `ps2_stubs_misc.inl:3318-3353` ANTES do `canCopyGuestByteRange`. Quando `xfer.dest==0xFFFFFFFF`, loga 1ª ocorrência por `src` único (mutex+`unordered_set`, máx 8) e usa `continue` SEM adicionar ao `pending` (não tenta copiar). Jogo recebe ID válido via `setReturnS32(allocateSifDmaTransferId())`. **⚠️ Sinal extra do log_part10b:** processo MORTO (linha 443 `jogar.sh, linha 29: 301110 Morto`) sem Ctrl+C, em VBlank #840 (vs #1680 do log_part10). Pode ser OOM killer Linux ou efeito colateral do flush pesado do dump em hex — investigar se persistir após B1. | Log mostrou exatamente `[PARTE 10 PLANO A] ... motivo='canCopyGuestByteRange_rejeitou_dest_ou_src'` (linha 400) + dump B3 (linhas 401-403, 407-411). Próximo log esperado: `[PARTE 10 PLANO B1] aceitando SIF RPC stage transfer (dest=0xffffffff, IOP decide pelo header) — xfer.src=0x... xfer.size=0x... xfer.attr=0x... ra=0x...` em vez do `[PARTE 10 PLANO A] failed`. Se jogo travar num poll de semáforo IOP em seguida → PARTE 11 / PARTE 10 PLANO B2 |
