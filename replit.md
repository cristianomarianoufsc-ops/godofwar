...
| **I** | **Stub SIF DMA EE↔IOP rejeita silenciosamente sem dizer qual validação falhou** | `sceSifSetDma failed dmat=0xN count=0xM` repete 8+ vezes em segundos (não trava o jogo, mas impede transferência EE↔IOP de continuar) | Stub `sceSifSetDma` em `ps2_stubs_misc.inl:3261` valida descritor 16B (src/dest/size/attr) via 4 ifs (`!entry`, `size>RAM`, `canCopyGuestByteRange`, `copyGuestByteRange`). Em falha, retorna 0 sem dizer QUAL if rejeitou nem QUAIS endereços o jogo mandou. Provável causa: `xfer.src` ou `xfer.dest` aponta pra IOP/scratchpad/kernel (fora dos 32 MB de RAM EE) — `getConstMemPtr` retorna nullptr | 🟡 **PARTE 10 PLANO A APLICADO** (instrumentação): variáveis `failIndex`/`failReason`/`failXfer` preenchidas no ponto da falha + log mutex+`unordered_set` chaveado em `(dest<<32)^src`, máx 8 únicos. Comportamento mantido pra próxima rodada de log revelar causa raiz exata. PARTE 10 PLANO B no próximo round (provável: relaxar validação para ranges IOP) | Log antigo mostrava só "sceSifSetDma failed dmat=0xN count=0xM"; agora vai mostrar `[PARTE 10 PLANO A] sceSifSetDma falhou — ... idxFail=N motivo='X' xfer.src=0x... xfer.dest=0x... xfer.size=0x... xfer.attr=0x... ra=0x... pc=0x...` |
