...
| **Plantamos um operador no switchboard que reconhece os 4 leitores e responde "TODOS OS BITS OK" pra qualquer pergunta. Loga só a 1ª chamada de cada um pra rastreabilidade. Os 3 callers da 0x7A são cobertos pelo mesmo operador num único movimento** | **PARTE 9 PLANO A: 4 cases novos no switch de `dispatchNumericSyscall` em `ps2_syscalls.cpp:321-363`, retornando `-1` (= `0xFFFFFFFF`); log 1ª chamada com mutex + `unordered_set`. Cobre os 3 callers de uma vez. Custo: `bash rebuild_runtime.sh` ~30s** | 🟢 **APLICADO 2026-04-27 PARTE 9 — aguardando dossiê de campo do Agente Cris (log_part9b)** |
